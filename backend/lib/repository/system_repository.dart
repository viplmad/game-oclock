...
import 'package:backend/entity/entity.dart'
    show SystemEntity, SystemID, PlatformID, SystemEntityData, SystemView;
...
class SystemRepository extends ItemRepository<SystemEntity, SystemID> {
  const SystemRepository(
    ...
    ImageConnector? imageConnector,
          ...
          itemConnector,
          imageConnector,
          recordName: SystemEntityData.table,
        );

  static const String _imagePrefix = 'icon';

  @override
  SystemEntity entityFromMap(Map<String, Object?> map) =>
      SystemEntity.fromMap(map);
  @override
  ...
  Future<List<SystemEntity>> findAllWithView(
  ...
  Future<List<SystemEntity>> findFirstWithView(
    SystemView systemView,
    int limit,
  ) {
  ...
  Future<List<SystemEntity>> findAllFromPlatform(PlatformID id) {
    final Query query =
        PlatformSystemRelationQuery.selectAllSystemsByPlatformId(id);
    return readItemList(
      query: query,
    );
  }
  ...
  Future<List<SystemEntity>> findFirstWithViewByName(
    SystemView systemView,
    String name,
    int limit,
  ) {
  ...
  //#region IMAGE
  Future<SystemEntity> uploadIcon(
    SystemID id,
  ...
  Future<SystemEntity> renameIcon(
    SystemID id,
    String imageName,
    String newImageName,
  ) {
