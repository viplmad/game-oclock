...
import './query/query.dart'
    show
        GameQuery,
        DLCQuery,
        GamePlatformRelationQuery,
        GamePurchaseRelationQuery,
        GameTagRelationQuery;
...
class GameRepository extends ItemRepository<GameEntity, GameID> {
  ...
  @override
  Future<List<GameEntity>> findAll([int? page]) {
    final Query query = GameQuery.selectAll(page);
    return readItemList(
      query: query,
    );
  }

  Future<List<GameEntity>> findAllOwned([int? page]) {
    return findAllOwnedWithView(GameView.main, page);
  }

  Future<List<GameEntity>> findAllRom([int? page]) {
    return findAllRomWithView(GameView.main, page);
  }
  ...
  Future<List<GameEntity>> findFirstWithView(
    GameView view,
    int limit,
    int? year,
  ) {
  ...
  Future<List<GameEntity>> findAllWithYearView(
    GameView view,
    int year, [
    int? page,
  ]) {
  ...
  Future<List<GameEntity>> findAllOwnedWithYearView(
    GameView view,
    int year, [
    int? page,
  ]) {
  ...
  Future<List<GameEntity>> findAllRomWithYearView(
    GameView view,
    int year, [
    int? page,
  ]) {
  ...
  Future<List<GameEntity>> findAllFromPlatform(PlatformID id) {
    final Query query =
        GamePlatformRelationQuery.selectAllGamesByPlatformId(id);
    return readItemList(
      query: query,
    );
  }
  ...
  Future<List<GameEntity>> findAllFromPurchase(PurchaseID id) {
    final Query query =
        GamePurchaseRelationQuery.selectAllGamesByPurchaseId(id);
    return readItemList(
      query: query,
    );
  }
  ...
  Future<Object?> unrelateGamePlatform(GameID gameId, PlatformID platformId) {
    final Query query =
        GamePlatformRelationQuery.deleteById(gameId, platformId);
    return itemConnector.execute(query);
  }

  Future<Object?> unrelateGamePurchase(GameID gameId, PurchaseID purchaseId) {
    final Query query =
        GamePurchaseRelationQuery.deleteById(gameId, purchaseId);
    return itemConnector.execute(query);
  }
  ...
  Future<List<GameEntity>> findFirstWithViewByName(
    GameView view,
    String name,
    int limit,
    int? year,
    ...
    final Query query =
        GameQuery.selectFirstInViewByNameLike(view, name, limit, year);
    return readItemList(
      query: query,
    );
  }
