...
import 'package:backend/model/model.dart' show DLC, Game, GameFinish, Item, Platform, Purchase, GameTag;
...
class GameRelationBloc<W extends Item> extends ItemRelationBloc<Game, GameID, W> {
  ...
  Future<List<W>> getRelationStream() {
    ...
    switch(W) {
      ...
      case Platform:
        final Future<List<PlatformEntity>> entityListFuture = platformRepository.findAllFromGame(id);
        return PlatformMapper.futureEntityListToModelList(entityListFuture, platformRepository.getImageURI) as Future<List<W>>;
      case Purchase:
        final Future<List<PurchaseEntity>> entityListFuture =  purchaseRepository.findAllFromGame(id);
        return PurchaseMapper.futureEntityListToModelList(entityListFuture) as Future<List<W>>;
      case GameTag:
        final Future<List<GameTagEntity>> entityListFuture = gameTagRepository.findAllFromGame(id);
