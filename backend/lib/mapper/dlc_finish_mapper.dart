...
class DLCFinishMapper {
  ...
  static Future<DLCFinish> futureEntityToModel(
    Future<DLCFinishEntity> entityFuture,
  ) {
    return entityFuture.asStream().map(entityToModel).first;
  }

  static Future<List<DLCFinish>> futureEntityListToModelList(
    Future<List<DLCFinishEntity>> entityListFuture,
  ) {
