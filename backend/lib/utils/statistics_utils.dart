class StatisticsUtils {
  static List<int> createIntervals(int chunk, int minReleaseYear, int maxReleaseYear) {
    int minYear;
  ...
  static List<int> intervalCount(List<int> intervals, Map<int, int> data) {
    ...
    for(int intervalIndex = 0; intervalIndex < intervals.length - 1; intervalIndex++) {
      ...
      final int intervalCount = data.entries
        .where((MapEntry<int, int> entry) => min < entry.key && entry.key <= max)
  ...
  List<int> intervalCountWithInitialAndLast(List<int> intervals, Map<int, int> data) {
    final List<int> values = List<int>.filled(intervals.length + 1, 0);
    int index = 0;
