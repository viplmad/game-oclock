...
class GameStatisticsView extends ItemStatisticsView<GameGeneralStatistics, GameYearStatistics, GameStatisticsBloc> {
  const GameStatisticsView({
    Key? key,
    required int viewIndex,
    required int? viewYear,
    required String viewTitle,
  }) : super(key: key, viewIndex: viewIndex, viewYear: viewYear, viewTitle: viewTitle);

  @override
  GameStatisticsBloc statisticsBlocBuilder(GameCollectionRepository collectionRepository) {
...
class _GameStatisticsBody extends ItemStatisticsBody<GameGeneralStatistics, GameYearStatistics, GameStatisticsBloc> {
  ...
  @override
  List<Widget> statisticsGeneralFieldsBuilder(BuildContext context, GameGeneralStatistics stats) {
    ...
    return <Widget>[
      ...
      statisticsDurationField(
        ...
        value: (playingPlayedCount > 0)? Duration(minutes: totalTime.inMinutes ~/ playingPlayedCount) : Duration.zero,
      ),
      ...
      const Divider(),
      _countByReleaseYearInIntervals(context, stats.minReleaseYear, stats.maxReleaseYear, stats.countByReleaseYear),
      const Divider(),
  ...
  @override
  List<Widget> statisticsYearFieldsBuilder(BuildContext context, GameYearStatistics stats) {
    final int totalItems = stats.total;
  ...
  //#region Common
  Widget _countByReleaseYearInIntervals(BuildContext context, int minReleaseYear, int maxReleaseYear, Map<int, int> data) {

    final List<int> intervals = StatisticsUtils.createIntervals(releaseYearIntervalChunk, minReleaseYear, maxReleaseYear);
  ...
  Widget _sumTimeByFinishDate(BuildContext context, Map<int, Duration> data) {
    ...
    return finishYears.isNotEmpty?
      statisticsHistogram<int>(
        ...
        domainLabels: domainLabels,
        values: data.values.map((Duration time) => time.inHours).toList(growable: false),
